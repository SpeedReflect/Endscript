...
{
	public class CarbonStream : BaseStream
	{
        private CarbonStreamingSection[] _sections;
        private string _directory;
		public override GameINT GameINT => GameINT.Carbon;
		public override string GameSTR => GameINT.Carbon.ToString();
        public CarbonStreamingSection[] Sections => this._sections;

		public CarbonStream(string lxry, string streamlxry) : base(lxry, streamlxry)
		{
            this._sections = new CarbonStreamingSection[0];
		}
		...
		public override void Load(string directory, string dest)
		{
            ...
            using (var br = new BinaryReader(File.Open(lxry, FileMode.Open, FileAccess.Read)))
			{
                ...
                while (br.BaseStream.Position < br.BaseStream.Length)
				{
                    ...
                    if (id == BinBlockID.TrackStreamingSections)
					{
                        ...
                        var count = size / 0x5C;
                        this._sections = new CarbonStreamingSection[count];

                        for (int i = 0; i < count; ++i)
                        {

                            this._sections[i] = br.ReadUnmanaged<CarbonStreamingSection>();
            ...
            using (var br = new BinaryReader(File.Open(streamlxry, FileMode.Open, FileAccess.Read)))
			{
                ...
                for (int i = 0; i < this._sections.Length; ++i)
				{
                    ...
                    File.WriteAllText(setsPath, CarbonStreamingSection.Serialize(section));
		...
		public override void Save(string directory, string src)
		{
            ...
            this._sections = new CarbonStreamingSection[sectionNames.Count];
            ...
            using (var bw = new BinaryWriter(File.Open(streamlxry, FileMode.Create, FileAccess.Write)))
            {
                ...
                for (int i = 0; i < sectionNames.Count; ++i)
                {
                    ...
                    var section = CarbonStreamingSection.Deserialize(File.ReadAllText(setsPath));
	...
	public struct CarbonStreamingSection
	{
        ...
        public static string Serialize(CarbonStreamingSection section)
        {
            return JsonSerializer.Serialize(section, options);
        }

        public static CarbonStreamingSection Deserialize(string data)
        {
            return JsonSerializer.Deserialize<CarbonStreamingSection>(data, options);
        }
    }
}
